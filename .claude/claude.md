# Threads自動投稿プロジェクト - Claude Code セッションガイド

## 🚨 新しいセッションで最初にすべきこと

### 1. プロジェクト構造の理解

#### 重要ファイル
- `data/posts_schedule.csv`: すべての投稿マスターデータ（永久保存、削除しない）
- `manuscripts/`: 短編小説の原稿ディレクトリ（番号管理、投稿分割前の完成原稿）
- `manuscripts/README.md`: 小説制作のガイドライン
- `.github/workflows/`: GitHub Actions自動投稿設定

#### 実行スクリプト
- 各種Python スクリプト（投稿生成、分析、修正など）

### 2. 短編小説制作フロー（manuscripts/）

#### 📚 コンセプトとテーマ

このアカウントでは、**教室を舞台にした短編小説**を連載しています。

**視点と文体:**
- 一人称（教師視点）
- 叙述: 常体（だ・である調）
- 会話: 丁寧語（です・ます調）

**構成:**
- 朝 → 午前 → 昼 → 午後 → 夕方 → 夜 → 深夜 → 翌日 → あとがき（先生の声）
- 各章の段落末に余韻を残し、後で24投稿などへ分割しやすい構造

**中心テーマ:**
教室で起きる小さな出来事（物、音、制度など）を通じて、子どもの背景にある家庭事情や心の問題に向き合う教師の物語。

**重要な原則:**
- 完璧な解決を描かない（妥協点や限界を示す）
- 教師の葛藤、迷い、無力感を率直に描く
- 格差、虐待、孤立、貧困など社会問題に触れる
- 親を一方的に悪者にしない（親の事情も描く）
- 個人特定情報は避け、観察に根拠を添える

#### 既存作品（2025-11-08時点）

1. **001_suzuno_oto.md（鈴の音）** - ランドセルの鈴と母の不安、子どもの選択権
2. **002_ibasho_no_nai_ko.md（居場所のない子）** - 虐待疑惑と通告の決断
3. **003_sensei_no_seigi.md（先生の正義）** - 体罰問題と教師間の価値観の対立
4. **004_tesuto_no_tensu.md（テストの点数）** - 成績と親の期待
5. **005_sankanbi_no_kaidan.md（参観日の階段）** - 親が来られない子どもへの配慮
6. **006_keshigomu_no_uso.md（消しゴムの嘘）** - 盗みの背景にある貧困
7. **007_hyakuten_no_kodoku.md（百点の孤独）** - 優等生の孤立
8. **008_sedai_no_kabe.md（世代の壁）** - ベテラン教師との世代間ギャップ
9. **009_juken_no_noroi.md（受験の呪い）** - 受験プレッシャー
10. **010_kyushoku_no_nokori.md（給食の残り）** - 食事ネグレクトと子どもの飢え
11. **011_hakui_no_nioi.md（白衣の匂い）** - 医療従事者の親を持つ子の寂しさ

#### 新作を作る際のチェックリスト

新しい小説を作成する前に、以下を確認:

1. **既存作品のテーマと重複していないか？**
   - [manuscripts/README.md](manuscripts/README.md) で既存テーマを確認
   - 既存の001-010の作品を確認

2. **教室の具体的なモチーフがあるか？**
   - ✓ 良い例: 鈴の音、消しゴム、給食、参観日など具体的な「物」や「出来事」
   - ✗ 悪い例: 抽象的なテーマ（「心の問題」「いじめ」など具体性がない）

3. **社会問題が自然に織り込まれているか？**
   - 説教臭くならない
   - 教師視点での観察と葛藤を中心に

4. **時系列構成が守られているか？**
   - 朝 → 午前 → 昼 → 午後 → 夕方 → 夜 → 深夜 → 翌日 → あとがき

5. **ファイル命名規則:**
   - `0XX_テーマ_ローマ字.md`（例: 010_kyushoku_no_nokori.md）
   - 番号は連番で管理（日付に依存しない）

#### note用タイトル作成の黄金ルール

note_articles/ ディレクトリに配置する記事タイトルは、以下のルールに従って作成:

**絶対原則:**
1. **「出オチ」を避ける** - タイトルだけでストーリーが見えてしまうのはNG
   - ✗ 悪い例: 「消しゴム100円を1ヶ月言い出せなかった子どもの窃盗」（詳細すぎる）
   - ✓ 良い例: 「消しゴムを盗んだ子どもの、貧困の重さ」（ミステリーがある）

2. **心理的トリガーを必ず含める** - 人が強烈に意識する要素:
   - 暴力（暴力、叩く、痣など）
   - 毒親（異常な教育、呪縛、支配など）
   - 貧困・金銭欲求（貧困、お金がないなど）
   - 承認欲求・愛情不足（愛されない、認められないなど）
   - 学歴コンプレックス（成績、受験など）
   - 性欲（性的な要素が関係する場合）

3. **実際の引用を使う** - ストーリー内に存在する子どもの衝撃的なセリフ
   - 必ず「」で囲む
   - 短く、印象的な言葉を選ぶ
   - 感情を揺さぶるものを優先

4. **ミステリーを作る** - 読まないとわからない要素
   - 「なぜ？」を想起させる
   - 「本当の理由」「失ったもの」などの曖昧な表現
   - 矛盾や意外性を含める

**タイトルフォーマット:**
```
# 「[子どもの衝撃的なセリフ]」[心理トリガー含む説明文] + [ミステリー要素]
```

**成功例:**

| NG（出オチ）| OK（ミステリー + トリガー）|
|------------|---------------------------|
| 消しゴム100円を1ヶ月言い出せなかった子どもの窃盗 | 「盗るつもりじゃなかった」消しゴムを盗んだ子どもの、**貧困**の重さ |
| 99点で泣く優等生の母の過剰な期待 | 「友達いらない」クラス1位の子どもが、母の**異常な教育**で失ったもの |
| 参観日に親が来なかった子の寂しさ | 「先生、見てくれてた?」親に**愛されない**子どもが、涙の理由 |
| 20点のテストで怒られる子どもの努力 | 「頑張ったねって嘘つかないでください」成績で子どもを測る母親の**呪縛** |
| 虐待されている子の通告判断 | 「先生、ここにいてもいいですか」家に帰らない子どもの体に、**痣**があった |
| 体罰教師との価値観の違い | 「**叩かなければ**分からない子もいる」と言い切る教師を、私は止められなかった |

**チェックリスト:**
- [ ] 「」付きの子どもの引用が含まれているか？
- [ ] 心理トリガー（暴力/毒親/貧困/承認欲求/学歴/性欲）のいずれかが含まれているか？
- [ ] タイトルだけでは結末がわからないか（出オチになっていないか）？
- [ ] 読みたくなる「謎」や「引っかかり」があるか？
- [ ] 具体的すぎる数字や詳細（「20点」「100円」「1ヶ月」など）を避けているか？

#### 新作小説の作成手順

```bash
# 1. 既存作品を確認
ls manuscripts/
cat manuscripts/README.md

# 2. 既存作品をいくつか読んでトーンを掴む
cat manuscripts/001_suzuno_oto.md
cat manuscripts/006_keshigomu_no_uso.md

# 3. 新しいテーマで執筆
# - 教室の具体的なモチーフから発想
# - 既存テーマと重複しないようチェック
# - 時系列構成を守る
# - 8000-9000文字程度
```

#### 投稿への分割ルール

**基本方針:**
- 完成した小説を15-25投稿程度に分割
- 各章の余韻を活かして自然な分割点を設定
- 感情的にサスペンスのある箇所で区切る
- Threads投稿フォーマットに変換して `data/posts_schedule.csv` に追加

**必須フォーマット要件:**

1. **投稿文字数**: 各投稿は最低140文字以上
   - 短すぎる投稿は避ける
   - 1投稿あたり200-400文字が理想

2. **改行の入れ方（重要）**:
   ```
   「会話文のあと改行」

   新しい場面や段落で改行。

   次の場面や出来事で改行。
   ```
   - 段落ごとに空白行を入れる
   - 会話と地の文の間に改行
   - 場面転換で必ず改行
   - 読みやすさを最優先

3. **投稿ID**: `0XX_YY` 形式
   - XX: ストーリー番号（001-049など）
   - YY: 投稿番号（01, 02, 03...）

4. **投稿スケジュール**:
   - 7:00~23:30の間で30分間隔
   - 日付をまたぐ場合は翌日の08:00から再開
   - 23:30の次は翌日08:00

5. **CSVフォーマット**:
   ```csv
   033_01,2025-12-04 16:30,"投稿本文

   段落改行を含む。

   このように改行を適切に入れる。",,pending,教室短編,ストーリー名,
   ```

**良い分割例:**
```
033_01,2025-12-04 16:30,"たくやが、今日も休んだ。三日連続だ。母親から「体調不良」と連絡があった。でも、先週も「体調不良」で休んでいた。

私は、たくやのことが気になっていた。一ヶ月前までは、毎日元気に登校していた。何かが、変わった。その「何か」を、私は知っていた。

一ヶ月前。たくやは、クラスの発表会でリーダーに選ばれた。「たくや、みんなをまとめてね」と私が言うと、たくやは「はい！」と元気に答えた。

でも、発表会の準備が始まると、問題が起きた。たくやの考えた案が、クラスで否定されたのだ。「たくやの案、つまらない」「もっと面白いのがいい」

子どもたちは、容赦なかった。たくやは、黙って下を向いた。",,pending,教室短編,逃げる瞬間,
```

**悪い分割例（改行なし）:**
```
033_01,2025-12-04 16:30,"たくやが、今日も休んだ。三日連続だ。母親から「体調不良」と連絡があった。でも、先週も「体調不良」で休んでいた。私は、たくやのことが気になっていた。一ヶ月前までは、毎日元気に登校していた。何かが、変わった。その「何か」を、私は知っていた。一ヶ月前。たくやは、クラスの発表会でリーダーに選ばれた。「たくや、みんなをまとめてね」と私が言うと、たくやは「はい！」と元気に答えた。",,pending,教室短編,逃げる瞬間,
```

**チェックリスト:**
- [ ] 各投稿140文字以上か？
- [ ] 適切な改行が入っているか？
- [ ] 感情的に引きのある箇所で区切っているか？
- [ ] 時刻は7:00-23:30の範囲で30分間隔か？
- [ ] CSV形式が正しいか（カンマ、引用符）？

### 3. データ管理

#### posts_schedule.csv の構造
```csv
id,datetime,text,thread_text,status,category,subcategory,hashtags
001_01,2025-11-08 15:00,"投稿本文",,pending,教室短編,鈴の音,
```

**重要:** このファイルはすべての投稿の永久記録です。削除しないでください。

#### カテゴリ体系
- **教室短編**: 短編小説シリーズ
- subcategory: 各作品のタイトル（鈴の音、消しゴムの嘘、など）

### 4. よくある作業パターン

#### 新しい小説の投稿データを生成
1. `manuscripts/0XX_title.md` を作成
2. Pythonスクリプトで24投稿に分割
3. `data/posts_schedule.csv` に追加
4. Git commit & push

#### トラブルシューティング
```bash
# CSVファイルの確認
head data/posts_schedule.csv

# 最新の投稿を確認
tail -20 data/posts_schedule.csv

# 特定のカテゴリを検索
grep "教室短編" data/posts_schedule.csv
```

---

## 📝 更新履歴

### 2025-11-08（小説コンテンツへの移行）
- **コンテンツ戦略の大転換**: 教室短編小説シリーズへ完全移行
- `manuscripts/` ディレクトリ作成
- 10作品の短編小説を制作
- 投稿を小説の分割投稿に切り替え
- CLAUDE.md を簡潔化（古い情報を削除）

### 2025-10-31
- ステートレスアーキテクチャ実装
- `posts_schedule.csv` を単一の情報源に
- GitHub Actions による自動投稿
