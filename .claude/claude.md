...
### 3. プロジェクト構造（2025-10-31 ステートレス化完了）
...
**仕組み:**
```
現在時刻 → タームjudge判定（8,12,15,18,21,23時）
    ↓
...
- `POSTING_GUIDELINES.md`: 投稿作成ガイドライン（短文・高頻度版、必読）
- `learnings.md`: 仮説検証ログ（PDCA改善履歴）

#### ブランチ戦略
- `main`: 唯一のブランチ（開発・実行全て）
- automationブランチは廃止済み

#### 実行スクリプト
- `threads_simple.py`: **メイン投稿スクリプト**
  - ステートレス設計
  - タームベース投稿判定
  - API照合による重複防止
  - `--dry-run`: テスト実行
...
# 昨日の投稿パフォーマンス
python3 threads_simple.py daily-report --dry-run
```
...
### 2025-10-31 午後（ステートレス化 + 短文・高頻度投稿）
- **完全ステートレス設計に移行**
  - `.last_posted_at` 廃止 → 状態管理完全廃止
  - タームベース判定導入（cron遅延±15分に対応）
  - API照合による重複防止
  - automationブランチ廃止 → mainのみ
  - リポジトリへの影響ゼロ化（ファイル書き込みなし）
- **投稿戦略を短文・高頻度に変更**
