...
毎朝9時（JST）に、前日の運用成果を自動でThreadsに投稿します。
...
- 毎日のフォロワー数を `daily_stats` テーブルに保存
- 前日比を自動計算
- 運用開始日（2025-10-29）からの日数を表示
...
| タイプ | 割合 | 特徴 | 文字数 |
|--------|------|------|--------|
| リアルタイム試行錯誤 | 29% | 進行中の挑戦記録 | 150-250字 |
| 体験談ベースノウハウ | 40% | 個人の物語として語る | 250-400字 |
| 役立つノウハウ | 31% | 実用的な情報 | 150-300字 |

### 🚨 スパム判定対策（必読）

**Threadsのスパム判定を避けるために:**
